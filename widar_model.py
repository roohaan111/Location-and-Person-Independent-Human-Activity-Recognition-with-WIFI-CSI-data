...
class RecognitionModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=22, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
        
        # Calculate the output size after convolutions and pooling
        # Initial size is (20, 20), apply Conv -> Pool -> Conv -> Pool sequence
        self.feature_map_height, self.feature_map_width = 20, 20  # starting dimensions
        self.feature_map_height = (self.feature_map_height - 3 + 2 * 1) // 1 + 1  # after conv1
        self.feature_map_width = (self.feature_map_width - 3 + 2 * 1) // 1 + 1
        self.feature_map_height = self.feature_map_height // 2  # after first pool
        self.feature_map_width = self.feature_map_width // 2
        self.feature_map_height = (self.feature_map_height - 3 + 2 * 1) // 1 + 1  # after conv2
        self.feature_map_width = (self.feature_map_width - 3 + 2 * 1) // 1 + 1
        self.feature_map_height = self.feature_map_height // 2  # after second pool
        self.feature_map_width = self.feature_map_width // 2
        
        # Flattened size to pass into fully connected layer
        self.feature_map_size = self.feature_map_height * self.feature_map_width * 128  # 128 is the output channels of conv3
        self.fc1 = nn.Linear(self.feature_map_size, 256)  # Example feature size for the fully connected layer
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.feature_map_size)  # Flatten for FC layer
        x = F.relu(self.fc1(x))
        return x


class StateMachineModel(nn.Module):
    def __init__(self, sequence_length=10, feature_dim=256, num_states=22):  # Use 22 as default if Widar dataset has 22 classes
        ...
        # Calculate the output dimension after convolutions and pooling
        reduced_dim = sequence_length // 2  # After max pooling with kernel size 2
        
        # Adjust input size to the fully connected layer based on final reduced dimensions
        self.fc1 = nn.Linear(128 * reduced_dim, num_states)  # Adjusted `num_states` based on dataset classes

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))  # Shape: (batch_size, 64, reduced_dim)
        x = F.relu(self.conv2(x))  # Shape: (batch_size, 128, reduced_dim)
        
        # Flatten along the remaining dimensions for final FC layer
        x = x.view(x.size(0), -1)  # Shape: (batch_size, 128 * reduced_dim)
        x = self.fc1(x)  # Shape: (batch_size, num_states)
        return x
...
class NASModel(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.lstm(x)
        x = self.fc(x[-1])  # Using the final time step for action prediction
        return x
